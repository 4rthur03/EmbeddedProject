...
static int __init startDriver(void)
{
	...
	if (alloc_chrdev_region(&Major, 0, 1, "relay") < 0)  //$cat /proc/devices
        {
	...
	if ( (cl = class_create(THIS_MODULE, "relay")) == NULL)	// $ls /sys/class
	{
		Log("2");
	...
	if ( device_create(cl, NULL, Major, NULL, "relay") == NULL) // $ls /dev/
	{
		Log("3");
		class_destroy(cl);
		unregister_chrdev_region(Major, 1);
...
static ssize_t device_read(struct file *filp, /* see include/linux/fs.h */
	...
	// TODO - Write code to handle transferring the desired data to the user
