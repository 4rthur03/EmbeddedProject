...
int main(int argc, const char *argv[])
{
	...
	if (mysql_real_connect(con, "localhost", "user", "user", "BrightnessDB", 0, NULL, 0) == NULL)
	{
		error_exit(con);
	}
	
	// Create the TempMeas database (if it doesn't already exist)
	if (mysql_query(con, "CREATE TABLE IF NOT EXISTS BrightData(BrightTime DATETIME, Brightness INTEGER)"))
	{
		error_exit(con);
	}
	...
	const char *query = "INSERT INTO BrightData(BrightTime, Brightness) VALUES(NOW(), ?)";
